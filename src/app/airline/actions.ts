...
/**
 * File an airline request from an already-signed-in account.
 *
 * The public registration above creates the account; this is the counterpart for
 * someone who already has one — retrying after a rejection, or linking an airline
 * for the first time. Same auto-verify rule (exact e-mail-domain match against
 * the selected airline). Because the account is by definition confirmed here, an
 * auto-approved request is granted its membership immediately, no activation
 * round-trip needed.
 */
export async function submitAirlineRequestAction(
  _prev: AirlineFormState,
  data: FormData,
): Promise<AirlineFormState> {
  const user = await requireUser("/airline");
  if (!user.emailConfirmed) {
    return { error: "Confirm your e-mail first — check your inbox for the link." };
  }
  ...
  if (airlineId) {
    ...
    const { data: openReq } = await supabase
      ...
      .maybeSingle();
    if (openReq) {
      return { error: "You already have a request pending on this airline." };
    }
  }
  ...
  try {
    ...
    if (matched && airlineId) {
      const { error: memberError } = await supabase
        .from("airline_members")
        .upsert(
          { airline_id: airlineId, user_id: user.id, role: "owner" },
  ...
  if (matched && airlineId) redirect("/airline");
  return { notice: "Sent for review — you'll get access as soon as it's approved." };
}
